...
class Testclass < SequelTestCase
	...
	context 'get - claimreport' do
		...
		def test_get_claimreport_valid
			get '/helpdesk/reports/claim' ,{page: 1, start: 0, limit: 25, token: @hd_token}
            assert_equal 200, last_response.status
			data = indifferent_data(JSON.parse(last_response.body))
			assert_not_nil data
		end

		def test_download_claimreport_csv
            get '/helpdesk/reports/claim/download', {token: @hd_token}
            assert_equal 200, last_response.status

        end

        def test_download_claimreport_csv_filter_daterange
			...
			get '/helpdesk/reports/claim/download', {token: @hd_token, filter: filter}
			assert_equal 200, last_response.status
		end
